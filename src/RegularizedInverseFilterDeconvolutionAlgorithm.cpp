...
Hyperstack RegularizedInverseFilterDeconvolutionAlgorithm::deconvolve(Hyperstack& data, PSF& psf) {
    ...
    for (auto& channel : data.channels) {
        ...
        fftw_complex* H2 = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * cubeVolume);
        fftw_complex* L = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * cubeVolume);
        fftw_complex* L2 = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * cubeVolume);
        fftw_complex* FA = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * cubeVolume);
        fftw_complex* FP = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * cubeVolume);
        ...
        for(auto& gridImage : split){
            ...
            // Perform complex division in frequency domain DECONVOLUTION
            //std::cout << "Performing Regularized Inverse Filter in frequency domain..." << std::endl;
            UtlFFT::complexMultiplication(padded_psfFFT, padded_psfFFT, H2, cubeVolume);
            //Laplacian L erzeugen
            UtlFFT::calculateLaplacianOfPSF(padded_psfFFT, L, cubeWidth, cubeHeigth, cubeDepth);
            UtlFFT::complexMultiplication(L, L, L2, cubeVolume);
            UtlFFT::scalarMultiplication(L2, this->lambda, L2, cubeVolume);

            UtlFFT::complexAddition(H2, L2, FA, cubeVolume);
            UtlFFT::complexDivision(padded_psfFFT, FA, FP, cubeVolume, this->epsilon);
            UtlFFT::complexMultiplication(image, FP, image, cubeVolume);
...
void RegularizedInverseFilterDeconvolutionAlgorithm::configure(const DeconvolutionConfig& config) {
    ...
    std::cout << "Configured RegularizedInverseFilter algorithm with lambda: " << this->lambda << std::endl;
    std::cout << "Configured RegularizedInverseFilter algorithm with epsilon: " << epsilon << std::endl;
    std::cout << "Configured RegularizedInverseFilter algorithm with grid: " << this->grid << std::endl;
    this->borderType = config.borderType;
    this->psfSafetyBorder = config.psfSafetyBorder;
    this->cubeSize = config.cubeSize;
    if(this->grid){
        std::cout << "Configured RegularizedInverseFilter algorithm with borderType: " << this->borderType << std::endl;
        std::cout << "Configured RegularizedInverseFilter algorithm with psfSafetyBorder: " << this->psfSafetyBorder << std::endl;
        std::cout << "Configured RegularizedInverseFilter algorithm with cubeSize: " << this->cubeSize << std::endl;
    }
}
