...
public:
    DeconvolutionBackendThreadManager(std::shared_ptr<ThreadPool> threadPool, size_t maxNumThreads, std::unique_ptr<DeconvolutionAlgorithm> algorithmPrototype, std::shared_ptr<IBackend> backendPrototype);
...
private:
    std::shared_ptr<IBackend> getBackend();
    void returnBackend(std::shared_ptr<IBackend> backend);
    ...
    std::vector<std::shared_ptr<IBackend>> unusedBackends;
    std::vector<std::shared_ptr<IBackend>> usedBackends;
