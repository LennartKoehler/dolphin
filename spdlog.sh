...
find "$ROOT" -type f \( "${find_args[@]}" \) | while IFS= read -r file; do
  ...
  perl -0777 -i.bak -pe '
    ...
    sub build_spdlog {
      ...
      my $level_enum =
        $level eq "error" ? "LogLevel::ERROR" :
        $level eq "warn"  ? "LogLevel::WARN"  :
        $level eq "info"  ? "LogLevel::INFO"  :
                            "LogLevel::DEBUG";

      my $out;
      if (@args) {
        $out = "g_logger(std::format(\"${fmt}\", " . join(", ", @args) . "), ${level_enum});";
      } else {
        $out = "g_logger(std::format(\"${fmt}\"), ${level_enum});";
      }
