...
public:
    using StrategyPairCreator = std::function<std::unique_ptr<DeconvolutionStrategyPair>(std::shared_ptr<SetupConfig>, std::shared_ptr<DeconvolutionConfig>)>;
     ...
     * @param setupConfig The setup configuration
     ...
     */
    std::unique_ptr<DeconvolutionStrategyPair> createStrategyPair(std::shared_ptr<SetupConfig> setupConfig, std::shared_ptr<DeconvolutionConfig> deconvConfig);

    /**
     * Create a deconvolution strategy based on DeconvolutionConfig (backward compatibility)
     * Automatically determines strategy type and loads required data (labeled images, PSF maps)
     * @param setupConfig The setup configuration
     * @param deconvConfig The deconvolution configuration
     * @return Unique pointer to the configured strategy, or nullptr if configuration is invalid
     */
    std::unique_ptr<IDeconvolutionStrategy> createStrategy(std::shared_ptr<SetupConfig> setupConfig, std::shared_ptr<DeconvolutionConfig> deconvConfig);
