...
double runSingleLargeFFTReal(const std::vector<float>& input,
    ...
    float* in = (float*)fftwf_malloc(sizeof(float) * dim_x * dim_y * dim_z);
    ...
    // Create plan
    fftwf_plan plan = fftwf_plan_dft_r2c_3d(dim_z, dim_y, dim_x,
                                       in, out, FFTW_MEASURE);
...
void runDecompositionTest(const TestConfig& config) {
    ...
    std::vector<float> large_input_real;
    for (auto x : large_input){
        large_input_real.push_back(x.real());
    }
    ...
    std::cout << "\n--- Testing Single Large FFT Real---" << std::endl;

    LIKWID_MARKER_INIT;
    LIKWID_MARKER_THREADINIT;
    LIKWID_MARKER_START("SingleLargeFFT");

    double large_fft_time_real = runSingleLargeFFTReal(large_input_real, large_output,
                                             config.large_dim_x, config.large_dim_y, config.large_dim_z,
                                             config.num_iterations, config.num_threads);

    LIKWID_MARKER_STOP("SingleLargeFFT");

    std::cout << "Single large FFT Real time: " << large_fft_time_real << " μs" << std::endl;
    std::cout << "Throughput: " << (1e6 / large_fft_time_real) << " FFTs/second" << std::endl;
