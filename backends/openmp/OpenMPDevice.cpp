...
#define FFTW_UNIFIED_CHECK(fftwf_result, operation) { \
    if ((fftwf_result) == nullptr) { \
        throw dolphin::device::DeviceException( \
...
#define FFTW_MALLOC_UNIFIED_CHECK(ptr, size, operation) { \
    if ((ptr) == nullptr) { \
        throw dolphin::device::MemoryException( \
...
void set_device_logger(LogCallback cb) {
    g_logger = cb;
}

// Static member definition
MemoryTracking OpenMPDeviceMemoryManager::memory;

extern "C" IDeconvolutionBackend* createDeconvolutionDevice() {
    return new OpenMPDeconvolutionBackend();
}

extern "C" IBackendMemoryManager* createBackendMemoryManager() {
    return new OpenMPDeviceMemoryManager();
}
...
// OpenMPDeviceMemoryManager implementation
OpenMPDeviceMemoryManager::OpenMPDeviceMemoryManager(){
...
void OpenMPDeviceMemoryManager::setMemoryLimit(size_t maxMemorySize) {
    std::unique_lock<std::mutex> lock(memory.memoryMutex);
    memory.maxMemorySize = maxMemorySize;
}

void OpenMPDeviceMemoryManager::waitForMemory(size_t requiredSize) const {
    std::unique_lock<std::mutex> lock(memory.memoryMutex);
    if ((memory.totalUsedMemory + requiredSize) > memory.maxMemorySize){
        throw dolphin::device::MemoryException("Exceeded set memory constraint", "OpenMP", requiredSize, "Memory Allocation");
    }
}

// OpenMPDeviceMemoryManager implementation
bool OpenMPDeviceMemoryManager::isOnDevice(void* ptr) const {
    // For OpenMP device, all valid pointers are "on device"
    return ptr != nullptr;
}

void OpenMPDeviceMemoryManager::allocateMemoryOnDevice(ComplexData& data) const {
...
ComplexData OpenMPDeviceMemoryManager::allocateMemoryOnDevice(const CuboidShape& shape) const {
    ComplexData result{this, nullptr, shape};
    allocateMemoryOnDevice(result);
    return result;
}

ComplexData OpenMPDeviceMemoryManager::copyDataToDevice(const ComplexData& srcdata) const {
...
ComplexData OpenMPDeviceMemoryManager::moveDataFromDevice(const ComplexData& srcdata, const IBackendMemoryManager& destDevice) const {
    BACKEND_CHECK(srcdata.data != nullptr, "Source data pointer is null", "OpenMP", "moveDataFromDevice - source data");
    if (&destDevice == this){
        return srcdata;
    }
    else{
        // For cross-device transfer, use the destination device's copy method
        return destDevice.copyDataToDevice(srcdata);
    }
}

ComplexData OpenMPDeviceMemoryManager::copyData(const ComplexData& srcdata) const {
...
void OpenMPDeviceMemoryManager::memCopy(const ComplexData& srcData, ComplexData& destData) const {
...
void OpenMPDeviceMemoryManager::freeMemoryOnDevice(ComplexData& data) const {
...
size_t OpenMPDeviceMemoryManager::getAvailableMemory() const {
    try {
        // For OpenMP device, return available system memory
        size_t memory = 0;
        #ifdef __linux__
...
size_t OpenMPDeviceMemoryManager::getAllocatedMemory() const {
...
void OpenMPDeconvolutionBackend::initializeGlobal() {
    ...
    if (!globalInitialized){
        fftwf_init_threads();
        //TODO how does this work which is globale tec. how do i do this with normal cpudevice, they kind of init the same fftw each?
...
void OpenMPDeconvolutionBackend::init(const BackendConfig& config) {
    initializeGlobal();
    if (g_logger) {
        g_logger("OpenMP device initialized for lazy plan creation", LogLevel::DEBUG);
    }
}
...
void OpenMPDeconvolutionBackend::cleanup() {
    ...
    destroyFFTPlans();
    if (g_logger) {
        g_logger("OpenMP device postprocessing completed", LogLevel::DEBUG);
    }
}
...
void OpenMPDeconvolutionBackend::forwardFFT(const ComplexData& in, ComplexData& out) const {
    ...
    if (planPair == nullptr) {
        std::unique_lock<std::mutex> lock(const_cast<OpenMPDeconvolutionBackend*>(this)->deviceMutex);
...
void OpenMPBackend::init(const BackendConfig& config){
    set_device_logger(config.loggingFunction);
